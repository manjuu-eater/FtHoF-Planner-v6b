 ...
 */
export const getBaseFailChance = () => {
    ...
    let failChance = 0.15;
    if (settings.buffDI)
        failChance *= 0.1; // Diminish Ineptitude Buff
    if (settings.debuffDI)
        failChance *= 5; // Diminish Ineptitude Debuff
 ...
 * @returns fail chance of FtHoF
 */
export const getFthofFailChance = (baseFailChance) => {
    const failChance = ((baseFailChance || getBaseFailChance())
 ...
 */
export const castFtHoF = (seed, spellsCastTotal, offset, isOneChange, forceCookie) => {
    // set seed (L312)
    ...
    if (isWin) {
        ...
        if (random2 < 0.25)
            choices.push("Building Special"); // Game.BuildingsOwned>=10 is ignored
        if (random3 < 0.15)
        ...
        if (random3 < 0.0001 && random4 >= 0.5) {
            ...
            if (!settings.buffDF)
                choicesIf.push("Click Frenzy");
            if (random1 < 0.1)
                choicesIf.push("Cookie Storm", "Cookie Storm", "Blab");
            //if (random2 < 0.25) choicesIf.push("Building Special");  // can omit with few buildings
            if (random2 < 0.15)
                choicesIf = ["Cookie Storm Drop"];
            if (random3 < 0.0001)
    ...
    if (!description)
        console.error("No description in dictionary: " + effectName);
    // add noteworthy info
    let noteworthy = false;
    if (effectName == "Building Special")
        noteworthy = true;
 ...
 */
export const hasCookieEffect = (cookies, effect) => {
    const effectNames = (typeof effect == "string" ? [effect] : effect);
    for (const cookie of cookies) {
        for (const effectName of effectNames) {
            if (cookie.name == effectName)
                return true;
        }
    }
    return false;
 ...
 */
export const castGFD = (seed, spellsCastTotal, offset) => {
    ...
    for (const spellKey in M_spells) {
        if (spellKey != "gambler's fever dream")
            spells.push(M_spells[spellKey]);
    }
